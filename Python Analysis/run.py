...
df_customer = pd.read_csv(customer_file_name, index_col=0).dropna(subset=['Latitude', 'Longitude'])
print(df_customer)

print("customer data loaded")
print("starting route optimization......")
transaction_df = df.drop_duplicates(subset=['CustomerCode', 'ProductCode'], keep='first').reset_index(drop=True)
