...
# Set up logging
logging.basicConfig(filename='../Logger/recommender_logs',
                    filemode='a',
...
def generate_recommender(df):
    try:
        ...
        logging.info("calculating cosine similarity score on total qty for every transaction......")
        ...
        logging.info("mapping top 10 products based on cosine similarity score.....")
        ...
        logging.info("mapping product name TH, group level 1 name, group level 2 name for each product input.....")
        ...
        logging.info("exporting recommendation to CSV......")
        recommendation_df.to_csv('recommendation.csv')

        end = datetime.now()

        logging.info( "Total time taken for recommendation: %s", end - start )
